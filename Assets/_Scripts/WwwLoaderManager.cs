...
public class WwwLoaderManager  {
	...
	private void OnProgressHandler(string path, float currentValue, float totalValue) {
		if (this.wwwLoaderOrder != null && this.wwwLoaderOrder.wwwLoaderOrderEvent != null ) {
			this.wwwLoaderOrder.wwwLoaderOrderEvent.InvokeLoaderProgress(path, currentValue, totalValue);
		}
	}
	...
	private void LoaderItem() {
		if (this.wwwLoaderOrder != null && this.wwwLoaderOrder.pathList.Count > 0) {
			...
			if(!existsStatus) {
				...
				this.wwwLoaderObject.Loader(this.wwwLoaderPath.path, 
											this.wwwLoaderPath.loaderTypeEnum,
											this.wwwLoaderPath.version,
											OnProgressHandler,
											OnCompleteHandler,
											OnErrorHandler);
			} else {
			...
			if (this.wwwLoaderOrder != null) {
				if (this.wwwLoaderOrder.wwwLoaderOrderEvent != null) {
					this.wwwLoaderOrder.wwwLoaderOrderEvent.InvokeLoaderComplete();
				}
	...
	/// <param name="currentValue">Current value.</param>
	/// <param name="totalValue">Total value.</param>
	private void OnProgreeeHandler(string path, float currentValue, float totalValue) {
		if (this.wwwLoaderOrder != null &&
			this.wwwLoaderOrder.wwwLoaderOrderEvent != null) {
			this.wwwLoaderOrder.wwwLoaderOrderEvent.InvokeLoaderProgress(path, currentValue, totalValue);
		}
	}
	...
	private WwwLoaderOrder GetWwwLoaderOrderByOrderName(string orderName) {
		...
		foreach(WwwLoaderOrder loaderOrder in this.orderList) {
			if (loaderOrder.orderName == orderName) return loaderOrder;
		}
		return null;
	}
}
